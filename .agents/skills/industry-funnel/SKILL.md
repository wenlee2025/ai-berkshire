...
This skill is generated from `skills/industry-funnel.md` for Google Antigravity.
...
```
第一层：美台全市场扫描 30-60 家   （活跃度+涨幅+市值前 30 的并集）
        ↓ 价值投资 5 条硬指标
...
**A 类 - 成交活跃度**：
- 近 30 天日均成交额排名行业前列（美股/台股各自取前 30）
...
| 市场 | 来源建议与工具 |
|------|---------------|
| 美股 | NASDAQ/NYSE 行业 ETF 持仓、Yahoo Finance、`tools/usstock_data.py` |
| 台股 | TWSE 上市/TPEx 上櫃 行业分类、Goodinfo、FinMind `tools/twstock_data.py` |
...
如果不想选股，列 1-3 个相关 ETF（美股/台股，如 QQQ, SOXX, 0050, 00830）。

### 5.3 整体行业位置判断

- 行业 PE/PB 历史分位
- 资金流向（ETF 申赎、机构持仓、卖方覆盖密度）
...
| 市场偏好 | 美股资料丰富，台股小巨人/隐形冠军容易被低估 | 必须美股与台股并重，台股供应链核心公司不能漏 |
