...
This skill is generated from `skills/industry-research.md` for Google Antigravity.
...
### 搜索清单
- 美股（NYSE/NASDAQ）相关上市公司（调用 `tools/usstock_data.py`）
- 台股（TWSE 上市 / TPEx 上柜）相关上市公司（调用 `tools/twstock_data.py`）
- 国际关键供应链核心企业
- 行业代表性 ETF（如 SOXX, SMH, QQQ, 0050 等）
- 关键未上市公司（潜在 IPO 候选）
