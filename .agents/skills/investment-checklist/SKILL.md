...
This skill is generated from `skills/investment-checklist.md` for Google Antigravity.
...
**支持输入格式**：单个或多个公司，用逗号/顿号/空格分隔。例如：`台积电, 英伟达, 微软` 或 `NVDA AAPL 2330.TW`
...
使用原生工具（`tools/usstock_data.py`, `tools/twstock_data.py`）及后台 Agent 为**每家公司**进行数据收集（所有公司同时并行启动），每个Agent负责收集：
...
**评分标准**（★1-5）：
- ★★★★★：商业模式极其简单清晰，10年确定性高（如可口可乐/台积电先进晶圆制造）
