...
This skill is generated from `skills/news-pulse.md` for Google Antigravity.
...
| **C 级（信息稀缺）** | 小市值、冷门股、新挂牌 | 转入"扫盲模式"——可能找不到任何能解释异动的新闻，**这个结论本身就有价值**（可能是技术性/资金面而非基本面） |
...
使用 TeamCreate 创建团队：
- `team_name`: `{公司名}-newspulse`（英文小写，如 `nvda-newspulse`）
...
- **description**:
  1. **官方公告**：SEC EDGAR（美股 8-K/10-Q/6-K）/ MOPS 公开资讯观测站（台股重大讯息/月营收）最近披露
  2. **财报与业绩指引**：最新季报/年报、业绩指引调整、电话会要点
  3. **管理层动作**：高管变动、内部人增减持（SEC Form 4 / 台股董监持股质押转让）、股票回购、分红政策
  4. **重大业务事件**：新产品发布、并购重组、大客户/大订单、产能扩张
  5. **资本运作**：再融资、可转债、ADR 转换与折溢价
  6. **诉讼与合规**：反垄断调查、专利诉讼、合规事件
...
- **description**:
  1. **行业监管**：美欧/台湾相关主管机关的新规、出口管制、实体清单
  2. **贸易与关税**：芯片法案补贴、关税调整、地缘政治与供应链重组
  3. **反垄断与竞争法**：FTC / DOJ / 欧盟委员会审查、调查与并购否决
  4. **货币与利率**：美联储议息、汇率波动（USD/TWD）、资本开支周期
  5. 每条政策标注：**日期 / 来源 / 直接影响该公司的程度（直接/间接/无关）**
  6. 重点判断：是否有"政策黑天鹅"刚刚落地
...
- **description**:
  ...
  2. **产业链上下游**：上游原材料/晶圆代工/设备、下游云巨头/终端客户，最近的价格、产能、订单变化
  3. **行业整体**：行业景气度数据、出货量、需求侧信号（CSP 资本开支、月营收数据等）
  4. **替代品威胁**：新技术、新架构对该行业的冲击
  5. **行业指数表现**：SOX 半导体指数、纳斯达克、台股电子指数最近表现，该公司是跑赢/跑输/同步
  6. 关键判断：**这是公司个体事件，还是整个行业的 beta 波动？**
  7. 每条事件标注来源和日期

#### 任务 4：市场情绪与机构观点（sentiment-tracker）

- **subject**: `侦察 {公司名} 近 {N} 天的市场情绪与机构观点变化`
- **description**:
  1. **卖方评级变动**：华尔街投行与台湾券商最近的评级与目标价调整
  2. **机构持仓变化**：美股 13F 披露、台股三大法人（外资/投信/自营商）买卖超
  3. **做空数据**：美股 Short Interest、台股借券卖出余额
  4. **市场讨论与观点**：专业论坛、投资人社区（Seeking Alpha、X/Twitter、Reddit）核心争议
  5. **技术面信号**：是否触及 52 周新高/新低、放量突破、均线乖离率
  6. 关键判断：**是基本面驱动还是情绪/资金面驱动？**
